...
| [AD](structured-vision-assessment.md) | Structured Vision Assessment | 🚧 Core shipped — schema-validated `analyzeFrame` sibling → typed `AssessmentCard` via forced tool-use + `vision_assess` + `instrument_reading` + first-aid triage consumers; Gemini `responseSchema` enforced via `GeminiSchemaTranslator` (51 tests). Deferred: CaptureFlow `voice_number` auto-fill |
