...
class GeminiLiveSessionManager: ObservableObject {
    ...
    private func buildSystemInstruction() -> String {
        ...
        if hasNativeTools || hasOpenClaw {
            ...
            - study: Study Mode — turn a saved document into flashcards + a quiz and review hands-free. Actions: make_deck (from 'deck' name or raw 'text'), list, quiz, answer (via 'value'), review, flip, grade (via 'value'), stop. Use for "make flashcards from X", "quiz me".
